...
router.get("/get", authToken, async function(req, res, next) {
    ...
    starCountRef.once("value", (snapshot) => {
        ...
        for (const property in data) {
            newData.push({
                ...
                image: `<img src="${data[property]["image"]}" style="width: 50px; height: 50px;" alt="">`,
                name: data[property]["name"],
                namegrouptype: data[property]["namegrouptype"],
    ...
    starCountRef.once("value", (snapshot) => {
        ...
        for (const property in data) {
            newData.push({
                ...
                name: data[property]["name"],
                button: `<button class="btn btn-danger" onclick="deleteUser('${property}') ">Delete</button>`,
            });
        }
        res.send({ data: newData });
    });
